...
StateManager::StateManager(SharedContext* l_shared)
	: m_shared(l_shared)
{
	RegisterState<load>(StateType::Load);
	CreateState(StateType::Load);
	m_loading = static_cast<load*>(m_states.back().second.get());
}
...
void StateManager::SwitchTo(const StateType & l_type) {
	...
	for (auto itr = m_states.begin(); itr != m_states.end(); ++itr)
	{
		...
		if (l_type == StateType::Load) { return; }
		if (m_loading->HasWork()) { SwitchTo(StateType::Load); }
		return;
	}
